...
function generateRestaurants(count) {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      image: "images/restaurants/" + cuisine + (i % 5 + 1) + ".jpg",
      ...
      images: Array(Math.floor(Math.random() * 6) + 3)
        .fill()
        .map((_, i) => "images/restaurants/" + cuisine + (i % 5 + 1) + ".jpg"),
...
function generateMenu(cuisine) {
  ...
  MENU_CATEGORIES.forEach((category) => {
    ...
    menu[category] = items.map((item) => ({
      ...
      image: "images/dishes/" + item.toLowerCase().replace(/\s+/g, "-") + ".jpg",
    }));
  });
...
function generateReviews(count) {
  const reviewers = [
    { name: "John D.", avatar: "images/avatars/avatar1.jpg" },
    { name: "Sarah M.", avatar: "images/avatars/avatar2.jpg" },
    { name: "Robert K.", avatar: "images/avatars/avatar3.jpg" },
    { name: "Emily L.", avatar: "images/avatars/avatar4.jpg" },
    { name: "Michael P.", avatar: "images/avatars/avatar5.jpg" },
    { name: "Jessica T.", avatar: "images/avatars/avatar6.jpg" },
  ];
      ...
      photos:
        Math.random() > 0.5
          ? Array(Math.floor(Math.random() * 3) + 1)
              .fill()
              .map((_, j) => "images/food/food" + (j % 5 + 1) + ".jpg")
          : [],
...
function setupEventListeners() {
  ...
  const restaurantMainImage = document.getElementById("restaurantMainImage");
  const restaurantImages = document.getElementById("restaurantImages");

  if (restaurantMainImage) {
    restaurantMainImage.addEventListener("change", (e) => handleImageUpload(e, "mainImagePreview"));
  }

  if (restaurantImages) {
    restaurantImages.addEventListener("change", (e) => handleImageUpload(e, "imagePreview"));
  }
  ...
  if (addRestaurantForm) {
    addRestaurantForm.addEventListener("submit", (e) => {
      ...
      if (mainImagePreview.children.length === 0) {
        alert("Please upload a main image for the restaurant.");
        return;
      }
      ...
      if (imagePreview.children.length < 3) {
        alert("Please upload at least 3 additional photos of the restaurant.");
        return;
      }
      ...
      const newRestaurant = {
        ...
        image: mainImagePreview.querySelector("img").src,
        rating: "0.0",
        reviewCount: 0,
        priceRange: "$",
        openingHours: "10:00 AM - 10:00 PM",
...
function handleImageUpload(e, previewId) {
  ...
  if (imagePreview) {
    // Clear previous previews for main image, keep for additional images
